          ...
          <div className="relative flex flex-col items-center">
            <div
              className={`w-5 h-5 rounded-full border-2 transition-colors
                ${index === currentStep || step.completed ? "border-red-500" : "border-gray-200"}
                ${index === currentStep && "ring-[3px] ring-red-500/20"}`}
            />
            {index < steps.length - 1 && (
              <div
                className={`w-[2px] h-[32px] mt-1 mb-1 transition-colors
                  ${index < currentStep || step.completed ? "bg-red-500" : "bg-gray-200"}`}
              />
            )}
          </div>
